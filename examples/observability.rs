//! Wiring observability for `wangcap-bridge`.
//!
//! Run with:
...
//!    `RUST_LOG="info,wangcap_bridge=debug,wacore=debug"`. The library groups
...
//! `pn#<keyed-token>`), so the `wangcap_bridge`/`wacore` log lines carry the same
...
fn main() {
    ...
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info,wangcap_bridge=debug"));
    ...
    // From here you would build and run a `wangcap_bridge::Client` as usual; all
